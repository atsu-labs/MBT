...
- 📱 **モバイル向け閲覧画面**: 
  ...
  - フィルター機能でステータス別に絞り込み
  - リスト表示と地図表示の切り替え
- 🗄️ **データ永続化**: ブラウザ LocalStorage（将来的に Cloudflare D1 へ移行予定）
- 🚀 **高速デプロイ**: Cloudflare Pagesで世界中にエッジ配信

## 技術スタック

- **フレームワーク**: React Router v7 (SPA モード)
- **UI**: React 19
- **地図**: Leaflet（動的インポート）
- **データストレージ**: ブラウザ LocalStorage（将来的に Cloudflare D1 へ移行予定）
