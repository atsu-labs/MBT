# アーキテクチャドキュメント

このドキュメントでは、MBT（マラソン・イベント管理システム）の技術アーキテクチャについて説明します。

## システム概要

MBTは、マラソンやイベントの事案管理、コース情報、救護所・関門・エイド・AED位置などを地図上で管理・表示するWebアプリケーションです。

**現在の実装状態**: React Router v7 の SSR モード（`ssr: true`）で動作しており、データは Cloudflare D1 に保存されます。

### 主要コンポーネント

```
┌─────────────────────────────────────────────────────────┐
│                      ユーザー                             │
└───────────────┬─────────────────┬───────────────────────┘
                │                 │
        ┌───────▼────┐    ┌──────▼──────┐
        │ PC管理画面  │    │ モバイル閲覧 │
        └───────┬────┘    └──────┬──────┘
                │                 │
                └────────┬────────┘
                         │
        ┌────────────────▼───────────────────┐
        │   React Router v7 (SSR モード)     │
        │   Cloudflare Workers               │
        └────────────────┬───────────────────┘
                         │
        ┌────────────────▼───────────────────┐
        │     Cloudflare D1 Database         │
        │     (SQLiteベースのサーバーレスDB)   │
        └────────────────────────────────────┘
```

## 技術スタック

### フロントエンド

- **React 19**: UIライブラリ
- **React Router v7**: フレームワーク（SSR モード、`ssr: true`）
  - `routes.ts` による明示的なルート設定（config-based routing）
  - サーバーサイドレンダリング（SSR）
  - `loader` / `action` によるサーバーサイドデータ操作
- **Leaflet**: 地図表示ライブラリ（SSR対応のため動的インポートで読み込み）
  - OpenStreetMapタイルレイヤー
  - カスタムマーカーとポップアップ
  - GeoJSONサポート
- **vis-timeline**: タイムライン表示ライブラリ（SSR対応のため `useEffect` 内で動的インポート）
  - グループ別タイムライン表示
  - ズーム・スクロール操作対応

### バックエンド

- **Cloudflare Workers**: サーバーレスエッジ実行環境（`workers/app.ts` がエントリーポイント）
- **Cloudflare D1**: SQLiteベースのサーバーレスデータベース
- **React Router loader/action**: サーバーサイドデータ取得・更新

### 開発ツール

- **TypeScript**: 型安全性
- **Vite**: ビルドツール
- **Wrangler**: Cloudflare開発CLI

## ディレクトリ構造

```
MBT/
├── app/                          # アプリケーションコード
│   ├── components/               # Reactコンポーネント
│   │   ├── Map.tsx              # Leaflet地図コンポーネント（SSR対応）
│   │   └── CaseMarker.tsx       # 事案マーカーアイコン生成
│   ├── data/                    # 静的データファイル
│   │   └── timeline.json        # タイムラインデータ（グループ・アイテム）
│   ├── lib/                     # ユーティリティとロジック
│   │   ├── context.ts           # AppLoadContext型定義（Cloudflare Workers用、DB・ASSETSバインディング含む）
│   │   ├── db.server.ts         # D1データベース操作
│   │   ├── types.ts             # TypeScript型定義
│   │   └── markers/             # マーカーデータ（GeoJSON）
│   │       ├── markers.ts       # 救護所、関門、エイド、AED
│   │       └── course.ts        # コース情報
│   ├── routes/                  # ルート定義
│   │   ├── _index.tsx           # トップページ
│   │   ├── admin.tsx            # 管理画面レイアウト
│   │   ├── admin._index.tsx     # ダッシュボード
│   │   ├── admin.cases._index.tsx    # 事案一覧
│   │   ├── admin.cases.new.tsx       # 事案作成
│   │   ├── admin.cases.$id.tsx       # 事案詳細
│   │   ├── admin.cases.$id.edit.tsx  # 事案編集
│   │   ├── mobile.tsx                # モバイル閲覧画面（地図最大化・ボトムシート・下部ナビ）
│   │   ├── mobile.help.tsx           # 使い方ページ（準備中）
│   │   ├── mobile.timeline.tsx       # 活動タイムライン画面
│   │   └── api.locations.ts          # 位置情報共有 REST API
│   ├── styles/                  # スタイルシート
│   ├── root.tsx                 # ルートコンポーネント
│   ├── entry.client.tsx         # クライアントエントリーポイント
│   ├── entry.server.tsx         # サーバーエントリーポイント
│   └── routes.ts                # ルート設定（config-based routing）
├── workers/                     # Cloudflare Workersエントリーポイント
│   └── app.ts                   # Workers ESMエントリーポイント
├── migrations/                  # D1データベースマイグレーション
│   ├── 0001_initial_schema.sql
│   └── 0002_add_user_locations.sql
├── docs/                        # ドキュメント
├── wrangler.toml               # Cloudflare Workers設定
├── vite.config.ts              # Vite設定
├── tsconfig.json               # TypeScript設定
└── react-router.config.ts      # React Router設定（ssr: true）
```

## データフロー

### 読み取り操作（loader）

```
1. ユーザーがページにアクセス
   ↓
2. Cloudflare Workers が React Router の loader を実行
   ↓
3. loader が db.server.ts の関数（例: getAllCases）を呼び出し
   ↓
4. D1 データベースからデータ取得
   ↓
5. React Router が SSR でHTMLを生成し、データをクライアントに送信
   ↓
6. クライアント側で useLoaderData() によりデータを利用
```

### 書き込み操作（action）

```
1. ユーザーがフォームを送信
   ↓
2. <Form method="post"> が HTTP POST リクエストを送信
   ↓
3. Cloudflare Workers が React Router の action を実行
   ↓
4. action が db.server.ts の関数（例: createCase）を呼び出し
   ↓
5. D1 データベースにデータ保存
   ↓
6. redirect() でページ遷移
```

## コンポーネント設計

### Map.tsx（地図コンポーネント）

**責務:**
- Leaflet地図の初期化と管理
- 事案マーカーの表示
- 固定マーカー（救護所、関門、エイド、AED）の表示
- コースラインの表示
- 地図クリックイベントの処理
- 親コンポーネントからの外部操作（ズーム・移動）の受け付け

**主要機能:**
1. **動的Leaflet読み込み**: SSR対応のため、`useEffect` 内でのみLeafletを動的インポート
2. **Leafletモジュール保持**: `useRef` で Leafletモジュールを保持し、`window.L` への依存を排除
3. **マーカー管理**: ES6 Mapを使用してマーカーを効率的に管理
4. **レイヤーコントロール**: 各種マーカーとコースの表示/非表示を切り替え
5. **カスタムアイコン**: 優先度に応じた色分けとサイズ調整
6. **ポップアップ**: 事案詳細情報の表示
7. **forwardRef + useImperativeHandle**: 親から `MapHandle` 経由でズーム・移動を操作可能

**Props:**
```typescript
interface MapProps {
  cases?: Case[];                    // 表示する事案の配列
  center?: [number, number];         // 地図の中心座標
  zoom?: number;                     // ズームレベル
  onMapClick?: (lat: number, lng: number) => void;  // 地図クリック時のコールバック
  selectedCaseId?: number;           // 選択中の事案ID
  userLocations?: UserLocation[];    // 共有中のユーザー位置情報の配列
  mySessionId?: string;              // 自身のセッションID（自分のマーカーを青色で表示）
}
```

**MapHandle（forwardRef 経由で親から呼び出せる API）:**
```typescript
export interface MapHandle {
  zoomIn: () => void;                                  // ズームイン
  zoomOut: () => void;                                 // ズームアウト
  flyTo: (lat: number, lng: number, zoom?: number) => void;  // 指定座標へ滑らかに移動
}
```

**使用例（親コンポーネントから地図を操作）:**
```tsx
const mapRef = useRef<MapHandle>(null);
<Map ref={mapRef} cases={cases} />

// ズームイン
mapRef.current?.zoomIn();
// 特定座標へ移動
mapRef.current?.flyTo(41.78, 140.74, 15);
```

> **注意**: Leafletの標準ズームコントロール（`zoomControl`）は `false` に設定されています。
> `/mobile` のカスタムズームFABと重複しないよう意図的に無効化しています。
> 管理画面など他の用途ではカスタムFABを使用するか、Map側で `zoomControl: true` に変更してください。

### mobile.timeline.tsx（活動タイムライン）

**責務:**
- vis-timeline を使った全スタッフの活動エリア・時間帯の可視化
- `app/data/timeline.json` からグループとアイテムを読み込む

**主要機能:**
1. **動的 vis-timeline 読み込み**: SSR対応のため、`useEffect` 内でのみ `import("vis-timeline")` を実行
2. **JSONデータ管理**: `app/data/timeline.json` にグループ（MBT1〜MBT17）とアイテムを JSON で定義
3. **表示日付の自動導出**: タイムライン軸の日付をハードコードせず、JSON の最初のアイテムから算出
4. **エリア配色**: CSS クラス（`area1`〜`area12`、`area345`、`area67`）でエリアを色分け
5. **localStorage 状態維持**: `/mobile` ↔ `/mobile/timeline` の遷移は通常のリンク遷移のため、`localStorage` の状態（`mbt_sessionId` 等）は自動的に保持される

**データ形式（`app/data/timeline.json`）:**
```json
{
  "groups": [
    { "id": "1", "content": "MBT1" },
    ...
  ],
  "items": [
    {
      "group": "1",
      "content": "area1-2",
      "className": "area1",
      "style": "font-size:10pt;",
      "start": "2026-06-28T09:14:00",
      "end":   "2026-06-28T10:01:00"
    },
    ...
  ]
}
```

**タイムラインデータの更新手順:**
1. `app/data/timeline.json` を編集する
2. `start`/`end` は ISO 8601 形式（`YYYY-MM-DDTHH:MM:SS`）で記述する
3. `npm run build` を実行してビルドを確認する
4. デプロイする（`npm run deploy`）

### mobile.tsx（モバイル閲覧画面）

**責務:**
- スマホ向けに最適化された地図最大化レイアウト
- 下部ナビゲーションバー（現在地 / タイムライン / 更新 / 事案一覧 / 使い方）
- ボトムシートによる事案一覧表示
- `MapHandle` 経由での地図操作（ズーム・フォーカス）
- 位置情報共有（パスコード認証付き）

**画面レイアウト:**

```
┌────────────────────────────────┐
│ MBT                  位置共有 ●│ ← ヘッダー（44px、濃紺背景）
├────────────────────────────────┤
│                                │
│           地図（全画面）         │
│                           [+] │ ← ズームFAB（右）
│                           [-] │
│                                │
├────────────────────────────────┤
│ 現在地 タイムライン 更新 事案 使い方│ ← 下部ナビ（safe-area対応）
└────────────────────────────────┘
```

**ボトムシート（事案一覧 タップ時）:**

```
┌────────────────────────────────┐
│   ────  （ドラッグハンドル）      │
│ 事案一覧（N件）  [すべて▼] [×] │
├────────────────────────────────┤
│ タイトル           [open][high]│
│ 概要テキスト...                │
│ YYYY/MM/DD HH:MM             │
├────────────────────────────────┤
│ ...                            │
└────────────────────────────────┘ ← 90vh
```

**主要機能:**
1. **ヘッダー**: "MBT" タイトルと位置共有トグルスイッチを最小高さ（44px）で表示。背景は濃紺（`#2c3e50`）
2. **地図最大化**: ヘッダー・下部ナビを除いた全高を占有
2. **ズームFAB**: 地図右に `+`/`-` ボタン、`MapHandle.zoomIn/zoomOut()` を呼び出し
3. **ボトムシート**: 事案一覧を90vh高さで表示。フィルタ・件数もシート内に集約
4. **事案一覧**: `created_at` 降順ソート（`useMemo` でメモ化）。タップで `flyTo` によるフォーカス
5. **下部ナビ**:
   - **現在地**: `navigator.geolocation` で位置取得し `MapHandle.flyTo()` で移動（権限拒否等のエラートースト表示）
   - **タイムライン**: `/mobile/timeline` へ遷移
   - **更新**: `useRevalidator().revalidate()` でloaderを再実行（ページリロードなし）
   - **事案一覧**: ボトムシートを開閉
   - **使い方**: `/mobile/help` へ遷移
   - **位置共有トグル**: ヘッダー右側に「位置共有」ラベルとトグルスイッチ。ON時は緑色、OFF時はグレーに変化

### mobile.help.tsx（使い方ページ）

**責務:**
- 「使い方」の準備中ページ（将来の詳細実装用プレースホルダー）
- 下部ナビの「使い方」から遷移

### ルートコンポーネント

各ルートコンポーネントは以下のパターンに従います（SSR + D1 パターン）：

```typescript
// loader でサーバーサイドからデータを取得
export async function loader({ context }: Route.LoaderArgs) {
  const cases = await getAllCases(context.cloudflare.env.DB);
  return { cases };
}

// action でサーバーサイドにデータを書き込み
export async function action({ request, context }: Route.ActionArgs) {
  const formData = await request.formData();
  await createCase(context.cloudflare.env.DB, {
    title: formData.get("title") as string,
    // ...
  });
  return redirect("/admin/cases");
}

// コンポーネントは useLoaderData でデータを利用
export default function Component() {
  const { cases } = useLoaderData<typeof loader>();
  return <div>{/* UI */}</div>;
}
```

## データベース設計

### casesテーブル

マラソンイベント中の事案（トラブル、救護要請など）を管理します。
`migrations/0001_initial_schema.sql` に定義されています。

**カラム:**
- `id`: 主キー（自動採番）
- `title`: 事案タイトル（必須）
- `description`: 詳細説明（オプション）
- `latitude`: 緯度（必須）
- `longitude`: 経度（必須）
- `status`: ステータス（"open" | "closed"）
- `priority`: 優先度（"high" | "medium" | "low"）
- `created_at`: 作成日時
- `updated_at`: 更新日時

**インデックス:**
- `idx_cases_status`: ステータスでの検索最適化
- `idx_cases_created_at`: 時系列でのソート最適化

### user_locationsテーブル

位置情報を共有中のユーザーのセッションを一時的に保存します。
`migrations/0002_add_user_locations.sql` に定義されています。

**カラム:**
- `session_id`: 主キー（クライアント側で生成したUUID）
- `user_name`: 地図上に表示するユーザー名（必須）
- `latitude`: 緯度（必須）
- `longitude`: 経度（必須）
- `updated_at`: 最終更新日時

**特記事項:**
- `getActiveLocations` では `updated_at >= datetime('now', '-10 minutes')` でアクティブなレコードのみを取得
- 位置共有オフ時にレコードを `DELETE` して除去することで、古いデータが残らないように設計

## マーカーデータ（GeoJSON）

### markers.ts

以下の固定マーカーデータを定義：
- **救護所（firstAid）**: 赤色マーカー
- **関門（gate）**: オレンジ色マーカー
- **エイド（aid）**: 緑色マーカー
- **AED**: 青色マーカー

各マーカーは以下の情報を持ちます：
```json
{
  "type": "Feature",
  "properties": {
    "name": "表示名",
    "comment": "コメント（オプション）"
  },
  "geometry": {
    "type": "Point",
    "coordinates": [経度, 緯度]
  }
}
```

### course.ts

マラソンコースを線で表示します：
```json
{
  "type": "Feature",
  "properties": {
    "name": "区間名",
    "_color": "#000000",
    "_opacity": 0.7,
    "_weight": 10
  },
  "geometry": {
    "type": "LineString",
    "coordinates": [[経度, 緯度], ...]
  }
}
```

## パフォーマンス最適化

### クライアントサイド

1. **動的インポート**: Leafletはクライアントサイドのみで読み込み（SSR対応）
2. **useRef でLeafletを保持**: `window.L` グローバル依存を排除し安定性向上
3. **マーカー管理**: ES6 Mapでマーカーを管理し、効率的な追加・削除を実現
4. **条件付きレンダリング**: ローディング状態を表示し、UX向上

### サーバーサイド

1. **SSRによる初期表示高速化**: サーバーでHTMLを生成して送信
2. **データベースインデックス**: 頻繁に検索するカラムにインデックスを作成
3. **最小限のデータ転送**: 必要なデータのみをクエリ
4. **エッジコンピューティング**: Cloudflare Workersで世界中のエッジから処理

## セキュリティ

1. **SQLインジェクション対策**: すべてのクエリでバインドパラメータを使用
2. **型安全性**: TypeScriptで実装し、実行時エラーを最小化
3. **サーバーサイドバリデーション**: `loader`/`action` で入力値を検証（タイトル必須、緯度経度NaN検証など）
4. **環境変数**: 機密情報は環境変数で管理
   - `LOCATION_SHARE_PASSCODE`: 位置情報共有APIのパスコード（本番環境では必ず設定すること）
5. **HTTPS**: Cloudflare Workersは自動的にHTTPSを提供

## スケーラビリティ

1. **サーバーレス**: Cloudflare Workersは自動スケーリング
2. **エッジキャッシング**: 静的アセットはエッジでキャッシュ
3. **D1データベース**: 自動的にレプリケーション
4. **グローバル配信**: Cloudflareのグローバルネットワークを活用

## デプロイメント

### ビルドプロセス

```bash
npm run build
# ↓
# Viteがアプリケーションをビルド
# ↓
# build/client/ に静的ファイル生成
# build/server/ にサーバーコード生成
```

### デプロイ先

- **サーバーサイド処理**: Cloudflare Workers（`workers/app.ts` → ビルド後は `build/server/`）
- **静的アセット**: Cloudflare Workers Assets（`build/client/`）
- **データストレージ**: Cloudflare D1

### CI/CD

GitHub連携により、mainブランチへのプッシュで自動デプロイが可能です。

## 開発環境

### ローカル開発

```bash
npm run dev
# ↓
# React Router Dev サーバー起動（SSRモード）
# Cloudflare D1 ローカルエミュレーション
# ↓
# http://localhost:5173 でアクセス
```

開発サーバーは以下の機能を提供：
- ホットモジュールリプレースメント（HMR）
- 自動リロード
- TypeScript型チェック
- エラーオーバーレイ
- D1ローカルエミュレーション（`wrangler` 経由）

### データストレージ

Cloudflare D1 ローカルエミュレーションを使用してデータを保存します。
`npm run db:migrate:local` でローカルDBにスキーマを適用してください。

## トラブルシューティング

### よくある問題

1. **Leafletが表示されない**
   - SSRとの互換性問題: `useEffect`内で動的読み込みを確認
   - CSS読み込み: Leaflet CSSが正しく読み込まれているか確認

2. **タイムラインが表示されない**
   - SSRとの互換性問題: vis-timeline は `useEffect` 内で `import("vis-timeline")` を使って動的読み込みしていることを確認
   - `app/data/timeline.json` の `items` 配列が空でないことを確認
   - `start`/`end` の形式が ISO 8601（例: `"2026-06-28T09:00:00"`）であることを確認

3. **D1接続エラー**
   - `wrangler.toml`のdatabase_idを確認
   - マイグレーションが実行されているか確認（`npm run db:migrate:local`）

4. **ビルドエラー**
   - `node_modules`と`.react-router`を削除して再インストール
   - TypeScript型エラーを確認

## 今後の拡張可能性

1. **リアルタイム更新**: WebSocketsまたはServer-Sent Eventsで事案の即時更新
2. **認証・認可**: 管理画面へのアクセス制御
3. **通知機能**: 新規事案や優先度の高い事案の通知
4. **データエクスポート**: CSV/JSON形式でのデータエクスポート
5. **モバイルアプリ**: Progressive Web App（PWA）化

## 参考リンク

- [React Router v7ドキュメント](https://reactrouter.com/)
- [Cloudflare Workersドキュメント](https://developers.cloudflare.com/workers/)
- [Cloudflare D1ドキュメント](https://developers.cloudflare.com/d1/)
- [Leafletドキュメント](https://leafletjs.com/)
